...
// matrix weight for the x and y components seperately. Each value us then
// added together, and the final sobel value is the lenght of the resulting float2.
...
void DepthSobel_float(float2 UV, float Thickness, out float Out) {
	...
	[unroll] for (int i = 0; i < 9; i++) {
		...
		sobel += depth * float2(sobelXMartix[i], sobelYMatrix[i]);
	}
